...
unsigned int nStakeMinAge = 8 * nOneHour; // 8 hour as zero time weight
unsigned int nStakeMaxAge = 30 * 24 * nOneHour; // 30 days as full weight
unsigned int nStakeTargetSpacing = 2 * 60; // 2-minute stakes spacing
int64_t nTargetSpacing = 2 * 60;  // Same as the above
unsigned int nModifierInterval = 12 * 60; // time to elapse before new modifier is computed (12 min)
...
const string strMessageMagic = "Cabbage Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Cabbage
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // Cabbage: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Cabbage", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Cabbage Genesis";
        CTransaction txNew;
        txNew.nTime = 1458763047;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1458763067;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2505083; // !fTestNet ? 1575379 : 46534;
       ...
       if (true && (block.GetHash() != hashGenesisBlock)) {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xcac61c8319de4ebc9cda514dfd27a89adfc3b4e0a8f4fed7f60d3091133c6775"));
...
unsigned char pchMessageStart[4] = { 0xb4, 0xf3, 0xe4, 0xd5 };
