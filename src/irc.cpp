...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CabbageTEST2\r");
            Send(hSocket, "WHO #CabbageTEST2\r");
        } else {
            // randomly join #Cabbage00-#Cabbage05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Cabbage%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Cabbage%02d\r", channel_number).c_str());
        }
